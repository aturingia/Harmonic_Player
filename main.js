...
const STORAGE_KEYS = {
    ...
    CURRENT_TIME: 'harmonicPlayer_currentTime',
    VOLUME: 'harmonicPlayer_volume',
    BACKGROUND_TYPE: 'harmonicPlayer_background_type',
    BACKGROUND_URL: 'harmonicPlayer_background_url',
    BACKGROUND_DATA: 'harmonicPlayer_background_data'
};
...
function applyBackgroundToBody(imageUrl) {
    const styleTag = document.getElementById('dynamic-bg-style');
    if (!styleTag) {
        const newStyle = document.createElement('style');
        newStyle.id = 'dynamic-bg-style';
...
async function saveLocalBackground(file) {
    ...
    if (file.size > 2 * 1024 * 1024) {
        showToast('La imagen es muy grande (máx 2MB). Usa una URL en su lugar.', true);
        return;
    }
    ...
    try {
        ...
        localStorage.setItem(STORAGE_KEYS.BACKGROUND_DATA, base64);
        localStorage.setItem(STORAGE_KEYS.BACKGROUND_TYPE, 'local');
        localStorage.removeItem(STORAGE_KEYS.BACKGROUND_URL);
        applyBackgroundToBody(base64);
        showToast('Imagen guardada permanentemente ✓');
    } catch (error) {
        console.error('Error:', error);
        showToast('Error al guardar la imagen', true);
    } finally {
        hideLoading();
    }
}
...
// Guardar fondo URL
function saveUrlBackground(url) {
    if (!url || !url.startsWith('http')) {
        showToast('Ingresa una URL válida (http:// o https://)', true);
        return;
    }

    applyBackgroundToBody(url);
    localStorage.setItem(STORAGE_KEYS.BACKGROUND_TYPE, 'url');
    localStorage.setItem(STORAGE_KEYS.BACKGROUND_URL, url);
    localStorage.removeItem(STORAGE_KEYS.BACKGROUND_DATA);
    showToast('Fondo URL guardado permanentemente ✓');
}
...
function resetDefaultBackground() {
    ...
    localStorage.setItem(STORAGE_KEYS.BACKGROUND_TYPE, 'default');
    localStorage.removeItem(STORAGE_KEYS.BACKGROUND_URL);
    localStorage.removeItem(STORAGE_KEYS.BACKGROUND_DATA);
...
async function restoreBackground() {
    const bgType = localStorage.getItem(STORAGE_KEYS.BACKGROUND_TYPE);

    if (bgType === 'url') {
        const url = localStorage.getItem(STORAGE_KEYS.BACKGROUND_URL);
        if (url && url !== 'null') {
            applyBackgroundToBody(url);
            return;
        }
    } else if (bgType === 'local') {
        const imageData = localStorage.getItem(STORAGE_KEYS.BACKGROUND_DATA);
        if (imageData) {
            applyBackgroundToBody(imageData);
            return;
        }
    }
}
...
function extractKeyFromFilename(filename) {
    ...
    for (let i = tokens.length - 1; i >= 0; i--) {
        ...
        if (camelotPattern.test(token)) return token;
    }

    let lastPart = nameWithoutExt.match(/([1-9]|1[0-2])(A|B)$/i);
    if (lastPart) return lastPart[0].toUpperCase();

    return null;
}

// Parsear clave Camelot
...
function getCompatibleKeys(key) {
    ...
    function addCompatible(num, let, type) {
        ...
        if (n < 1) n = n + 12;
        const keyStr = `${n}${let}`;
        if (!compatibles.has(keyStr)) compatibles.set(keyStr, type);
    }
...
function getMixSuggestions(track, allTracks) {
    ...
    for (let i = 0; i < allTracks.length; i++) {
        ...
        if (compatType) {
            suggestions.push({ index: i, name: other.name, key: other.key, type: compatType });
        }
    }
    return suggestions;
}
...
function renderPlaylist() {
    if (playlist.length === 0) {
        playlistContainer.innerHTML = '<div class="empty-playlist">🎵 No hay canciones. Carga archivos de audio 🎵</div>';
        return;
    }
        ...
        div.innerHTML = `
            ...
            ${suggestions.length > 0 ? `
                <div class="harmonic-suggestions">
                    ...
                    ${suggestions.map(sug => `
                        <span class="harmony-tag ${sug.type}" data-index="${sug.index}">
        ...
        div.querySelectorAll('.harmony-tag').forEach(tag => {
            tag.addEventListener('click', (e) => {
                ...
                if (!isNaN(idx)) {
                    loadTrack(idx);
                    setTimeout(() => playAudio(), 100);
                    showToast(`🎵 Mezclando con: ${playlist[idx].name}`);
                }
            });
        });
...
function playAudio() {
    ...
    if (playPromise !== undefined) {
        playPromise.then(() => {
            isPlaying = true;
            playPauseBtn.innerHTML = '<i class="bx bx-pause-circle"></i>';
            saveSession();
        }).catch(() => {
            isPlaying = false;
            playPauseBtn.innerHTML = '<i class="bx bx-play-circle"></i>';
        });
    }
}
...
async function handleAudioFiles(files) {
    ...
    for (let i = 0; i < validFiles.length; i++) {
        ...
        try {
            ...
            playlist.push({ name, key, blobUrl, fileName: file.name });
            completed++;
        } catch (error) {
...
fileInput.addEventListener('change', (e) => {
    if (e.target.files.length) handleAudioFiles(Array.from(e.target.files));
    fileInput.value = '';
});
...
playPauseBtn.addEventListener('click', () => {
    ...
    if (audio.paused || !isPlaying) playAudio();
    else pauseAudio();
});
...
resetBgBtn.addEventListener('click', () => {
    if (confirm('¿Restaurar el fondo original?')) {
        resetDefaultBackground();
    }
});

localBgInput.addEventListener('change', async (e) => {
    const file = e.target.files[0];
    if (file) {
        await saveLocalBackground(file);
    }
    localBgInput.value = '';
});
...
statusBadge.addEventListener('click', () => {
    if (playlist[currentTrackIndex]?.key) {
        ...
        showToast(`🎛️ ${suggestions.length} mezclas compatibles disponibles`);
    } else {
...
window.addEventListener('beforeunload', () => {
    if (audio.currentTime) localStorage.setItem(STORAGE_KEYS.CURRENT_TIME, audio.currentTime);
});
...
async function init() {
    ...
    loadSession();
    console.log('🎵 Harmonic Player cargado correctamente');
    showToast('🎨 El fondo se guarda permanentemente');
}
